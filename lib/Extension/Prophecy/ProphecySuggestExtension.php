        ...
        $container->register('prophecy.suggest', function (Container $container) {
            return new PhpactorComposerSuggestor(
                ...
                function (JsonConfig $config, ComposerInspector $inspector) {
                    if ($config->has(ProphecyExtension::PARAM_ENABLED)) {
                        return Changes::none();
                    }
                    ...
                    return Changes::from([
                        new PhpactorConfigChange('Prophecy mocking framework detected, enable Prophecy extension?', function (bool $enable) {
                            return [
                                ProphecyExtension::PARAM_ENABLED => $enable,
                            ];
                        })
                    ]);
                }
